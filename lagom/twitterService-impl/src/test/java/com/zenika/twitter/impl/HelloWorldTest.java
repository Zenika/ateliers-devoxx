...
public class HelloWorldTest {
  ...
  public void testHelloWorld() {
    ...
    Outcome<HelloEvent, WorldState> outcome1 = driver.run(new HelloCommand.Hello("Alice", Optional.empty()));
    assertEquals("Hello, Alice!", outcome1.getReplies().get(0));
    assertEquals(Collections.emptyList(), outcome1.issues());

    Outcome<HelloEvent, WorldState> outcome2 = driver.run(new HelloCommand.UseGreetingMessage("Hi"),
        new HelloCommand.Hello("Bob", Optional.empty()));
    assertEquals(1, outcome2.events().size());
    Assert.assertEquals(new HelloEvent.GreetingMessageChanged("Hi"), outcome2.events().get(0));
